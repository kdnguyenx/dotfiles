...
vim.api.nvim_create_autocmd('filetype', {
  pattern = 'netrw',
  group = vim.api.nvim_create_augroup('armletznetrw', { clear = true }),
  callback = function()
    vim.opt_local.colorcolumn = ''
...
vim.api.nvim_create_autocmd({ 'bufwritepost', 'vimleavepre' }, {
  pattern = '*',
  group = vim.api.nvim_create_augroup('armletzsessionizer', { clear = true }),
  callback = function()
...
vim.api.nvim_create_autocmd('vimenter', {
  pattern = '*',
  group = vim.api.nvim_create_augroup('armletzautosession', { clear = true }),
  callback = function()
    if #vim.v.argv > 3 then
      return
    end
