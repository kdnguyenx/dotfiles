...
vim.api.nvim_create_autocmd('filetype', {
  pattern = 'netrw',
  group = vim.api.nvim_create_augroup('armletnetrw', { clear = true }),
  callback = function()
    vim.opt_local.colorcolumn = ''
...
vim.api.nvim_create_autocmd({ 'bufwritepost', 'vimleavepre' }, {
  pattern = '*',
  group = vim.api.nvim_create_augroup('armletsessionizer', { clear = true }),
  callback = function()
...
vim.api.nvim_create_autocmd('vimenter', {
  pattern = '*',
  group = vim.api.nvim_create_augroup('armletautosession', { clear = true }),
  callback = function()
    if #vim.v.argv > 3 then
      return
    end
